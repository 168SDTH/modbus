...
class modbus(QWidget):
    ...
    async def _run(self):
        server = await asyncio.start_server(self.autosend, self.host, self.port)
    ...
    async def autosend(self, reader, writer):
        # 异步读取串口数据
        ...
        # 异步写入串口数据
        async def write_to_serial(writer, data):
        ...
        while True:
            # 等待主站发送数据
            read = asyncio.create_task(read_from_serial(reader))
            await read
            data = read.result()
            # 05 运行
            if data == bytes.fromhex(f"010500000001"):
                write = asyncio.create_task(
                    write_to_serial(writer, bytes.fromhex(f"010500000001"))
                )
                await write
                self.running()
                # 根据当前状态将运行次数写入寄存器
            ...
            # 05 停止
                ...
                self.stop()
                write = asyncio.create_task(
                    write_to_serial(writer, bytes.fromhex(f"010500000002"))
                )
                await write
                status = False
            # 根据寄存器中数据显示当前运行次数
            ...
            if data == bytes.fromhex(f"010100000001"):
                if status:
                    write = asyncio.create_task(
                        write_to_serial(writer, bytes.fromhex(f"010100000001"))
                    )
                    await write
                    print("running")
                else:
                    write = asyncio.create_task(
                        write_to_serial(writer, bytes.fromhex(f"010100000002"))
                    )
                    await write
                    print("stop\n")
            ...
            # 03 主站读取寄存器中运行次数信息
                ...
                # send = send_data + bytes.fromhex(f"{self.modbus_crc16(send_data):04X}")
                write = asyncio.create_task(write_to_serial(writer, send_data))
                await write
...
if __name__ == "__main__":
    app = QApplication([])
    m = modbus(host="0.0.0.0", port=1235)
